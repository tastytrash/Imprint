...
public class FootprintParticle extends SingleQuadParticle {
    ...
    private static final int ANIMATION_DELAY = 55;

    public FootprintParticle(ClientLevel level, double x, double y, double z, double velX, double velY, double velZ, float yaw, SpriteSet sprites) {
        super(level, x, y, z, 0.0, 0.0, 0.0, sprites.first());

        this.sprites = sprites;
        this.lifetime = ImprintClient.config.footprintLifetime;
        this.quadSize = 0.1875f * (float) ImprintClient.config.particleSettings.scale;
        this.gravity = 0.0f;
        ...
        alpha *= ImprintClient.config.particleSettings.alpha / 255.0f;
        this.setAlpha(alpha);
    ...
    public void tick() {
        ...
        this.setSprite(this.sprites.get(Math.max(0, this.age - ANIMATION_DELAY), this.lifetime - ANIMATION_DELAY));
    ...
    public record Factory(SpriteSet sprites) implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NonNull SimpleParticleType parameters, @NonNull ClientLevel world, double x, double y, double z, double velocityX, double velocityY, double velocityZ, @NonNull RandomSource random) {
            ...
            return new FootprintParticle(world, x, y, z, velocityX, velocityY, velocityZ, yaw, sprites);
        }
    }
}